...
| Markdown 渲染 | 表格 / 代码块 / 加粗 / 链接等格式正常显示，不再是一堆乱文本 |
| 思考模式 / 附件上传 | 发送时可勾选思考模式、附带文件（各平台支持度不同） |
...
| Kimi | 🔧 需登录（常驻窗口） | Kimi 会校验浏览器实例，登录后窗口保持打开才能对话 |
| 智谱（ChatGLM） | ✅ 可用 | chatglm.cn，游客模式即可对话 |
| 元宝 | 🔧 需登录（常驻窗口） | 必须微信 / 手机 / QQ 登录后才能对话 |
| 豆包 | 🔧 需登录（常驻窗口） | 登录后窗口保持打开才能对话；输入框已校准 |
...
>
> Kimi / 豆包 / 元宝 使用「常驻窗口」模式：登录成功后浏览器窗口会保持打开，
> 聊天也在同一个窗口进行（这些网站会校验浏览器实例，换窗口登录即失效）。
> 请勿手动关闭这些常驻窗口，否则需要重新登录。

### 发送选项（思考模式 / 模型 / 附件）

- **思考模式**：勾选后会在发送前尝试开启各平台的「深度思考 / 思考」开关
- **模型**：各平台模型列表待校准，目前默认使用网页默认模型
- **附件**：选择一个文件后随消息一起上传（需要平台支持上传）
